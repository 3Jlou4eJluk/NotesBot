...
@add_note_router.message(Command('add_note'))
...
async def get_picture(message :types.Message, command :CommandObject):
    if command.args:
        ...
        response = await get_request(ml_api_link + '/' + 'get_picture')
        plot_encoded_string = response.json()['plot']
        plot_extension = response.json()['file_extension']
